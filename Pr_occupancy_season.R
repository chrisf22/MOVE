...
##### the proportion of individuals who likely crossed into the specified extent; 'time_step' only includes 
##### uncertainty from estimating locations for each time step in the movement model; 'fast' starts with 
##### posterior means to get rapid point estimates

Pr_occupancy_season <- function(posts_lat, posts_lon, lat_input_min, lat_input_max, lon_input_min, lon_input_max, side, n.iter, N, season_length, method){
  
  #ptm <- proc.time()
  lamin <- (posts_lat - lat_input_min) > 0
  lamax <- (lat_input_max - posts_lat) > 0
  lomin <- (posts_lon - lon_input_min) > 0
  lomax <- (lon_input_max - posts_lon) > 0
  ...
  if(method=="time_step_bymo"){
    ...
    #mo <- calendar[which(calendar[,2]==first_rec_day_mo[,1]&calendar[,1]==first_rec_day_mo[,2]):length(calendar[,1]), 2]
    ...
    inorout_summed_mo <- inorout_summed_mo/N[mo_index]
  ...
  if(method=="JAGS"){
    ...
    for(i in 1:n.iter){
      ...
      bin_jags <- jags(data=data, inits=inits, parameters.to.save=parameters, filename,
                       n.chains=1, n.burnin=1000, n.iter=2000, n.thin=1, DIC=TRUE)
  ...
  if(method=="individual"){
    ...
    #pr_byind_alt <- apply(inorout_summed, 1, sum)/N
